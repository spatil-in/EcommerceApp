...
export default function Home(){
    ...
    useEffect(() => {
        ...
        .then(([data1, data2]) => {
            ...
            const formatData1 = data1.products.map(product => ({
                ...product,
                thumbnail: product.thumbnail,
                uniqeID: `api1-${product.id}`
            }));

            const formatData2 = data2.map(product => ({
                ...product,
                thumbnail: product.image,
                uniqeID: `api2-${product.id}`
            }));
    ...
    return(
        <section className="py-5">
         ...
         <div className="max-w-[1320px] grid xl:grid-cols-4 lg:grid-cols-3 md:grid-cols-2 mx-auto gap-5 mt-5 px-7">
            {data.map((product) => (
                <ProductCard key={product.uniqeID} product={product} />
            ))}
         </div>
         <ToastContainer/>
        </section>
...
function ProductCard({product}) {
    ...
    const {title , price , id, thumbnail , description , uniqeID} = product
    ...
    const checkcartItem = cart.find((obj) => obj.uniqeID == uniqeID)
